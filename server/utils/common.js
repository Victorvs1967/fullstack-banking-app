...
const getTransactions = async (account_id, start_date, end_date) => {
    ...
    try {
        if (start_date && end_date) {
            ...
            result = await pool.query(
                "select to_char(transaction_date, 'YYYY-MM-DD') as formatted_date, withdraw_amount, deposit_amount, balance from transactions account_id=$1 order by transaction_date desc",
                [account_id]
            );
        }
        return result;
...
const generatePdf = async filepath => {
    ...
    await page.addStyleTag({
        ...
        headerTemplate: `
            <div style="font-suze: 7px; white-space: nowrap; margin-left: 38px;">
            ${moment(new Date()).format('Do MMMM YYYY')}
            </div>
        `,
        footerTemplate: `
            <div style="font-size: 7px; white-space: nowrap; margin-left: 38px; margin-right: 35px; width: 100%;">
